---
type: home
tags:
  - wiki
created: 2026-08-23
updated: 2026-08-23
---

# Home

Door to the vault. Open this folder in Obsidian. Then open graph view.

## Result

Nine X posts compiled into a wiki you can browse as a graph.

- Gold: [[wiki]]
- Teal: [[maps]]
- Blue: [[hunt]] / [[twitter]]
- Green: [[andrej-karpathy]], [[jacky-kwok]], [[rohit]]
- Coral: [[ship]]

Open `output/obsidian-graph.html` for the dark dataflow. Canvas: `maps/dataflow.canvas`. Snapshot: `output/obsidian-graph.svg`. Compiled mermaid: [[graph]]. Visual brief: [[dataflow-visual-2026-08-23]]. Live open check: [[obsidian-open-2026-08-23]].

## Start

- [[Today]]
- [[index]]
- [[agent-operating-system]]
- [[contradictions]]
- [[Jarvis]]
- [[TELOS]]

## Do this next

- Drop a file in `raw/` and say `ingest this`.
- File a clip in [[inbox]], then ingest.
- Say `lint the wiki`.

## This batch

2026-08-17 to 2026-08-22, compiled 2026-08-23. Brief: [[ingest-brief-2026-08-23]].
