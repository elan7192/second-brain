...
If you are not in Obsidian, run `python3 tools/render-obsidian-graph.py` and open `output/obsidian-graph.html` (concept islands). Dark compile bus: `python3 tools/render-dataflow.py`. Renders are derived and gitignored. In Obsidian: dark theme, then `maps/dataflow.canvas` or graph view.
