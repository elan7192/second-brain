...
Open `output/obsidian-graph.html` if you are not in Obsidian. Dark page: zoom, pan, hover or click a node for title and wiki path. Macro zoom shows island headers. Node labels appear on zoom-in without overlap. Static copies: `output/obsidian-graph.svg`, `output/obsidian-graph.png`. See [[graph-ui-2026-08-24]].
