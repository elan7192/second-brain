---
id: source:src-fhwofjow-book-to-skill
type: source
tags:
  - twitter
created: 2026-09-04
updated: 2026-09-04
---

# src-fhwofjow-book-to-skill

- URL: https://x.com/fhwofjow51260/status/2095488892309049627
- Author: 娜美知识库 (@fhwofjow51260)
- Tweet: 2095488892309049627
- Date: 2026-09-03 12:27 UTC
- Retrieved via FxTwitter syndication. Thick pointer ingest (入vault建議). Quote untrusted. Principles only — no tweet dump. Not copied into `raw/`.

## Claims kept

book-to-skill (https://github.com/virgiliojr94/book-to-skill): turn a book/doc into a callable Skill.

Inputs: PDF / EPUB / DOCX (also papers, manuals, internal docs). Output: structured Skill (chapters, core knowledge, terms, methods) so the agent can call knowledge without re-stuffing hundreds of pages each turn.

Principle: read once → permanent skill, not re-read-the-book every session. Do not auto-vendor; compile SOPs into wiki/skills locally. See [[skill-as-sop]] · [[skill-library]].

## Pages updated

[[skill-as-sop]] · [[skill-library]]
