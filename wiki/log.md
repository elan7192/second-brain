...
New [[src-andrewng-ai-engineering-skills-map]] from @AndrewYNg tweet 2095890279865721217. 判定=入vault. Kept (quote untrusted): plan→exec→deploy/monitor; scarce skills spec/arch/verify; five hire skills (names not invented here); harness wraps model; autonomy dial interactive/delegated/looped (loop needs testable goal+stop); four fail modes overengineer / no verification / stops short / destroys data. Secondary recap cite only: @polydao 2095999364988538886 (no separate pointer). Related cross-link: [[src-pvncher-rethinking-skills-gpt6-astra]] · [[src-rutin-astra-routine-prompt-pattern-2026-09]] · [[src-spotify-portal-claude-cheap-workers]] · [[harness-routing]] · [[skill-as-sop]]. No deep concept-body folds. Catalog row added. Squash-merge if validate PASS.
