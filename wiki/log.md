...
New [[src-andrewng-ai-engineering-skills-map]] from @AndrewYNg tweet 2095890279865721217 (+ Batch companion). 判定=入vault. Kept (quote untrusted): plan→exec→deploy/monitor; scarce skills = spec/arch/verify; five hire skills named in primary (directing the workflow; enabling agent autonomy; reviewing the work; customizing the agent and its environment; coding agent foundations); harness wraps model; autonomy dial interactive/delegated/looped (loop needs testable goal+stop); four fail modes overengineer / no verification / stops short / destroys data. Secondary recap cite only: @polydao 2095999364988538886 (no separate pointer). Related only (no deep concept-body folds): [[src-pvncher-rethinking-skills-gpt6-astra]] · [[src-rutin-astra-routine-prompt-pattern-2026-09]] · [[src-spotify-portal-claude-cheap-workers]] · [[harness-routing]] · [[skill-as-sop]]. Catalog row added. Squash-merge if validate PASS.

## [2026-09-05] ingest | Spotify Portal Claude cheap-workers primary (promote)

Researchy VERIFIED TRUE: Spotify published. New canonical primary [[src-spotify-portal-claude-cheap-workers]] from https://engineering.atspotify.com/2026/9/portal-by-spotify-cut-my-claude-code-token-usage-by-90 (Dimitri Mazmanov, 2026-09-03) + https://github.com/spotify/portal-ai-plugins. Kept: file→summary (bulk-read / cheap AiKA); example→disk (code-write); hard gate SHUNT_MIN_LINES default 350; ~90% = mean bulk-read savings on Java monorepo tests — not company-wide bill; do not conflate Anthropic Spotify case study (migration time). DROP prior "unverified" / cite-only framing for the Spotify publish claim. Rewrote [[src-undefinedki-spotify-claude-cheap-workers]] into thin SECONDARY pointer → primary (Yarchi @undefinedKi discovery cite only). Related fix on [[src-avichawla-trueforge]]. Near keep (no deep concept-body folds): [[src-community-compaction]] · [[harness-routing]] · [[work-per-cost]] · [[tokens-as-capital]]. Catalog row added/updated. Squash-merge if validate PASS.
...
Profiled every CLI path before touching code. Three structural costs: link resolution in `index.py` was a correlated subquery with ten OR branches per link (54% of rebuild); `pathlib.relative_to` was 28% of validate; the CLI imported every subcommand module for `ask`. Fixed those plus FTS5 external content, indexes on the join tables, an atomic temp-file rebuild, batched neighbor fetches, lazy snippets, one search per eval question, and an injection-regex prefilter. Warm-cache medians: rebuild-index 253 to 93 ms, ask 48 to 31 ms, eval 397 to 136 ms. Note for anyone timing on this VM: the stdlib `.pyc` files are stale and unwritable, so every process recompiles 32 stdlib modules; set `PYTHONPYCACHEPREFIX` before comparing numbers. Eval exposed one real ranking gap while bisecting a recall drop caused by earlier wiki edits: a page found by FTS and cited by the top claim got no claim-link bonus. Fixed; recall back to 95%. Ontology checked with `python3 tools/ontology.py verify`: 430 objects, 3024 links, 0 dangling, 2270 page edges identical to the FTS index. Found and fixed: `outbound` wrong on 24 objects, 209 links outside the schema's own `linkTypes` (schema widened to WikiNote, check now enforced), `builtAt` churn. See [[vault-ontology]] and [[musk-algorithm]].
